...
- Custom steering files: !`bash -c 'if [ -d ".kiro/steering" ]; then count=$(find .kiro/steering -maxdepth 1 -type f -name "*.md" ! -name "product.md" ! -name "tech.md" ! -name "structure.md" | grep -c .); if [ "$count" -gt 0 ]; then echo "🔧 $count custom file(s) found - Will be preserved"; else echo "📋 No custom files"; fi; else echo "📋 No steering directory yet"; fi'`
